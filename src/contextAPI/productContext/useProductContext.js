...
const UseProductContext = ({ children }) => {
  ...
  const [images, setImages] = useState([
    { url: "/gold_neckless.png" },
    { url: "/gold_pendent.png" },
    { url: "/sankha.jpg" },
    { url: "/silver pendent.jpg" },
    { url: "/nose_ring.png" },
  ]);

  const addWishlistCart = async (product) => {
    // product = {productId, source}
    try {
      setLoader(1);
  ...
  const fetchWishlistCart = async (source) => {
    // get wishlist / cart
    try {
      setLoader(1);
  ...
  const removeWishlistCart = async (product) => {
    // remove wishlist / cart
    try {
      setLoader(1);
  ...
  const checkWishlistCart = async (source, prodId) => {
    // get wishlist / cart
    try {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const options = {
          ...
          currency: "INR",
          image:
            "https://t4.ftcdn.net/jpg/02/50/57/61/360_F_250576190_w8XwVQmucKcdyRkNBR4p2S641hrMkmbX.jpg",
  ...
  const fetchImages = async () => {
    try {
      const getCollections = await fetch(`${BASE_URL}api/getBlobs`);
      const allCollections = await getCollections.json();
      console.log(allCollections)
      if (allCollections.success) {
        setImages([...images, ...allCollections.blobs]);
      }
    } catch (e) {
      console.log("error to fetch collections");
    }
  };
